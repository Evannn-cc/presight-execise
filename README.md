...
- **Facet semantics**: top-20 counts reflect the active text filter and selected filters. The hobby facet applies _all_ filters including its own selections — with AND semantics each count is a drill-down preview of "results if you add this hobby". The nationality facet excludes its _own_ selections (while still applying search + hobbies): nationalities are OR semantics, so self-filtering would hide every unselected value and make multi-nationality selection impossible through the UI. Selected values are always rendered in the sidebar even when they drop out of the top 20, so they stay removable.
